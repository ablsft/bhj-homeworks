class Game {
  ...
  registerEvents() {
    ...
    function checkSymbol(e) {
      let symbolFromDom = this.currentSymbol.textContent.toLowerCase();
      let symbolFromKeyboard = e.key.toLowerCase();

      if (symbolFromKeyboard.length === 1) {
        if (symbolFromDom === symbolFromKeyboard) {
          this.success();
        } else {
          this.fail();
        }
